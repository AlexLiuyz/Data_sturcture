...
class Graph {
    ...
    private:
        ...
        vector<Node> Graph::dijkstra(char name[]) {
            ...
            Node* boundary=(Node*)malloc(sizeof(Node));
            boundary=NULL;
            while (!pq.empty()) {
                Node* u=(Node*)malloc(sizeof(Node));
                u = pq.top().second; // extract node with smallest distance
                pq.pop();
                boundary=u;
                for(vector<pair<Node*,pair<char*,int> > >::iterator neighbor=u->neighbors_.begin();neighbor!=u->neighbors_.end();neighbor++){
                    Node* v = search_by_name(neighbor->first->name_);
                    int weight = neighbor->second.second;//edge distance
            ...
            for (Node* u = boundary; u != NULL; u = prev[u - graph.data()]) {
                path.push_back(*u);
            }
        ...
        void Graph::change_relationship(char command[]){
            ...
            if(strcmp(component,"rm")==0){
                ...
                while(true&&node!=NULL){
                    ...
                    char *temp;
                    printf("input command(e.g name-宋江,placeoforigin-兰州,age-80)for changing field, please use small case\n");
                    cin>>new_command;
                ...
                printf("Warning: The person you want to edit does not exist!\n");
            }
        }
//print the relationship between person
...
//build relationship between two different person
        void Graph::add_edge(char name1[],char name2[],char relationship[]) {
            ...
            for (vector<pair<Node*,pair<char*,int> > >::iterator neighbor=node1->neighbors_.begin();neighbor!=node1->neighbors_.end();neighbor++) {
                if(strcmp(neighbor->first->name_,node2->name_)==0){
                    printf("Warning: Already build relationship!\n");
                    return;
                }
            }
